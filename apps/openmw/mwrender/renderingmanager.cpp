...
namespace MWRender
{
    ...
    void RenderingManager::update(float dt, bool paused)
    {
        ...
        // 128x128 over 24576 u (192 u texels, as the 64x64 over 12288 u it
        // rebaked when the camera crosses 1024 u (16384 getHeightAt calls,
        // the same query actor placement uses). Missing
        ...
        if (osg::Image* shore = mWater->getShoreImage())
        {
            ...
            constexpr float extent = 24576.f;
            constexpr int texels = 128;
            ...
            if (!mShoreBaked || (snapped - mShoreBakeOrigin).length2() > 1.f)
            {
                ...
                for (int y = 0; y < texels; ++y)
                    for (int x = 0; x < texels; ++x)
                        px[y * texels + x] = mTerrain->getHeightAt(osg::Vec3f(
                            origin.x() + (x + 0.5f) * (extent / texels),
                            origin.y() + (y + 0.5f) * (extent / texels), 0.f));
                shore->dirty();
